...
def _find_project_root(start: Path, max_depth: int = 8) -> str:
    ...
    for _ in range(max_depth):
        ...
        if (candidate / "src" / "wellbeing_pipeline").exists():
            return str(candidate)
...
def bootstrap(project_root: str = None) -> Path:
    """
    Add src/ to sys.path so the wellbeing_pipeline package imports cleanly in notebooks.

    Args:
        project_root: Absolute repo root. If None, tries env vars or notebook path.

    Returns:
        Path to the wellbeing_pipeline package directory.
    """
    ...
    pkg_dir = src_dir / "wellbeing_pipeline"
